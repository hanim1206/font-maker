...
function toPathD(shape: Shape): string {
  ...
  if (isPath(shape)) {
    ...
    for (let i=0;i<nodes.length-1;i++){
      ...
      if (a.h2 && b.h1) p.bezierCurveTo(a.h2.x, a.h2.y, b.h1.x, b.h1.y, b.p.x, b.p.y);
    ...
    if (isCubic(shape)) p.bezierCurveTo(shape.c1!.x, shape.c1!.y, shape.c2!.x, shape.c2!.y, shape.p1.x, shape.p1.y);
    else p.lineTo(shape.p1.x, shape.p1.y);
  }
  return p.toString();
}
function reverseStroke(s: Stroke): Stroke { return { ...s, p0: s.p1, p1: s.p0, c1: s.c2, c2: s.c1 }; }

// 큐빅 분할(De Casteljau)
function splitCubic(p0:Pt,c1:Pt,c2:Pt,p1:Pt,t:number){
  const p01=lerp(p0,c1,t), p12=lerp(c1,c2,t), p23=lerp(c2,p1,t);
...
// Handle
function Handle({
  p, onDrag, onDragEnd, label, fill="#fff", stroke="#111"
}:{
  p: Pt;
  label?: string;
  fill?: string;
  ...
  const bind = useGesture(
    {
      ...
      onDrag: ({ delta: [dx, dy], shiftKey, event }) => { event?.stopPropagation(); onDrag(dx, dy, { shiftKey }); },
      onDragEnd: ({ movement: [mx, my], shiftKey, event }) => { event?.stopPropagation(); onDragEnd(mx, my, { shiftKey }); },
    },
...
export default function ShapeEditor() {
  ...
  const [shapes, setShapes] = useState<Shape[]>([
    { type:'stroke', id:crypto.randomUUID(), p0:{x:120,y:240}, p1:{x:260,y:240}, stroke:'#111', strokeWidth:18, strokeLinecap:'round', strokeLinejoin:'round' },
    { type:'stroke', id:crypto.randomUUID(), p0:{x:260,y:240}, p1:{x:260,y:360}, c1:{x:260,y:260}, c2:{x:260,y:340}, stroke:'#111', strokeWidth:18, strokeLinecap:'round', strokeLinejoin:'round' },
  ]);
  ...
  const addStroke = () =>
    setShapes(prev => [...prev, {
      ...
      stroke: '#111', strokeWidth: 18, strokeLinecap:'round', strokeLinejoin:'round'
    }]);
  ...
  const addEllipse = () => {
    addEllipseInternal(canvasW/2, canvasH/2, Math.round(canvasW*0.32), Math.round(canvasH*0.2));
  };

  const addEllipseInternal = (cx:number, cy:number, rx:number, ry:number) => {
    const kx = KAPPA * rx;
    const ky = KAPPA * ry;

    const n0: Node = { p:{x:cx+rx, y:cy},    h1:{x:cx+rx, y:cy-ky}, h2:{x:cx+rx, y:cy+ky} }; // right
    const n1: Node = { p:{x:cx,    y:cy+ry}, h1:{x:cx+kx, y:cy+ry}, h2:{x:cx-kx, y:cy+ry} }; // bottom
    const n2: Node = { p:{x:cx-rx, y:cy},    h1:{x:cx-rx, y:cy+ky}, h2:{x:cx-rx, y:cy-ky} }; // left
    const n3: Node = { p:{x:cx,    y:cy-ry}, h1:{x:cx-kx, y:cy-ry}, h2:{x:cx+kx, y:cy-ry} }; // top
  ...
  const toggleCubic = () => {
    ...
    setShapes(prev => prev.map(s => {
      ...
      if (isCubic(s)) { const { c1, c2, ...rest } = s; return rest; }
  ...
  const mergeSelected = () => {
    ...
    setShapes(prev => [...prev.filter(s => s.id !== a.id && s.id !== b.id), newPath]);
    setSelected(newPath.id);
    setMode('points');
  };

  // ── Stroke 핸들 이동
  const movePoint = (shapeId: ID, key: 'p0'|'p1'|'c1'|'c2', dx: number, dy: number, meta:{shiftKey:boolean}) => {
    setShapes(prev => prev.map(s => {
  ...
  const commitPoint = (shapeId: ID, key: 'p0'|'p1'|'c1'|'c2', meta:{shiftKey:boolean}) => {
    setShapes(prev => prev.map(s => {
      if (isPath(s) || s.id !== shapeId) return s;
      if ((key === 'c1' || key === 'c2') && !isCubic(s)) return s;
      const cur = (s as any)[key] as Pt | undefined;
  ...
  // ── Path 노드/핸들 드래그
  const movePathP = (shapeId:ID, idx:number, dx:number, dy:number, meta:{shiftKey:boolean})=>{
    setShapes(prev=>prev.map(s=>{
      if (!isPath(s) || s.id!==shapeId) return s;
      const n = s.nodes[idx]; if (!n) return s;
      const raw = { x:n.p.x+dx, y:n.p.y+dy };
      const next = meta.shiftKey ? snapClampByKey('p0', raw) : clampByKey('p0', raw);
      const nodes = s.nodes.slice(); nodes[idx] = { ...n, p: next };
      return { ...s, nodes };
    }));
  };
  const movePathH = (shapeId:ID, idx:number, which:'h1'|'h2', dx:number, dy:number, meta:{shiftKey:boolean})=>{
    setShapes(prev=>prev.map(s=>{
      if (!isPath(s) || s.id!==shapeId) return s;
      const n = s.nodes[idx]; if (!n) return s;
      ...
      const next = meta.shiftKey ? snapClampByKey('c1', raw) : clampByKey('c1', raw);
      const nodes = s.nodes.slice(); nodes[idx] = { ...n, [which]: next } as Node;
      return { ...s, nodes };
    }));
  };
  const commitNoop = (_mx:number,_my:number,_m:{shiftKey:boolean})=>{};
  ...
  const bindCanvas = useGesture({
    ...
    onDrag: ({ movement: [mx, my], buttons, shiftKey, event }) => {
      ...
      if (shiftKey) { sdx = snap(anchor.x + mx) - anchor.x; sdy = snap(anchor.y + my) - anchor.y; }

      const anchors: Pt[] = isPath(start) ? start.nodes.map(n => n.p) : [(start as Stroke).p0, (start as Stroke).p1];
      ...
      setShapes(prev => prev.map(s => {
        ...
        if (isPath(start) && isPath(s)) {
          const nodes = s.nodes.map((_,i) => ({
            p:  { x: (start as PathShape).nodes[i].p.x  + sdx, y: (start as PathShape).nodes[i].p.y  + sdy },
            h1: (start as PathShape).nodes[i].h1 ? { x: (start as PathShape).nodes[i].h1!.x + sdx, y: (start as PathShape).nodes[i].h1!.y + sdy } : undefined,
            h2: (start as PathShape).nodes[i].h2 ? { x: (start as PathShape).nodes[i].h2!.x + sdx, y: (start as PathShape).nodes[i].h2!.y + sdy } : undefined,
          }));
          return { ...s, nodes };
        } else if (!isPath(start) && !isPath(s)) {
          const p0 = { x: (start as Stroke).p0.x + sdx, y: (start as Stroke).p0.y + sdy };
          const p1 = { x: (start as Stroke).p1.x + sdx, y: (start as Stroke).p1.y + sdy };
          const next: Stroke = { ...s, p0: clampByKey('p0', p0), p1: clampByKey('p1', p1) };
          if (isCubic(start as Stroke)) {
            const c1 = { x: (start as Stroke).c1!.x + sdx, y: (start as Stroke).c1!.y + sdy };
            const c2 = { x: (start as Stroke).c2!.x + sdx, y: (start as Stroke).c2!.y + sdy };
    ...
    onDragEnd: () => { dragRef.current.start = null; dragRef.current.shapeId = null; },
  });

  // 선택 변경 시 드래그 스냅샷 폐기
  useEffect(() => { dragRef.current.start = null; dragRef.current.shapeId = null; }, [selected]);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if ((e.key.toLowerCase() === 'c') && (e.ctrlKey || e.metaKey)) {
        if (selectedShape) clipboardRef.current = JSON.parse(JSON.stringify(selectedShape));
        return;
      }
      // paste
      if ((e.key.toLowerCase() === 'v') && (e.ctrlKey || e.metaKey)) {
        ...
        if (!clip) return;
        const dup = offsetShape(withNewId(clip), PASTE_OFFSET, PASTE_OFFSET, clampByKey);
      ...
      if ((e.key === 'Backspace' || e.key === 'Delete') && selected) { e.preventDefault(); removeSelected(); }
      else if (e.key.toLowerCase() === 'c' && !e.ctrlKey && !e.metaKey) toggleCubic();
      else if (e.key.toLowerCase() === 'm') mergeSelected();
      else if (e.key.toLowerCase() === 'a') addNodeMid();
      else if (e.key.toLowerCase() === 'e') setMode(m=> m==='select' ? 'points' : 'select');
  ...
  const updateStyle = (patch: Partial<Pick<Shape,'stroke'|'strokeWidth'|'strokeLinecap'|'strokeLinejoin'>>) => {
    if (!selectedShape) return;
    setShapes(prev => prev.map(s => s.id === selectedShape.id ? { ...s, ...patch } as Shape : s));
  };

  const getBBoxCenter = (s: Shape): Pt => {
    const pts: Pt[] = isPath(s) ? s.nodes.map(n => n.p) : [(s as Stroke).p0, (s as Stroke).p1];
  ...
  const scaleSelected = (factor: number, scaleStrokeWidth: boolean) => {
    ...
    const scalePt = (p: Pt): Pt => ({ x: center.x + (p.x - center.x) * factor, y: center.y + (p.y - center.y) * factor });
    ...
    setShapes(prev => prev.map(s => {
      ...
      if (isPath(s)) {
        ...
        return { ...s, nodes, strokeWidth: scaleStrokeWidth ? (s.strokeWidth ?? 4) * factor : s.strokeWidth };
      } else {
        const p0 = clampByKey('p0', scalePt(s.p0));
        const p1 = clampByKey('p1', scalePt(s.p1));
        const next: Stroke = { ...s, p0, p1, strokeWidth: scaleStrokeWidth ? (s.strokeWidth ?? 4) * factor : s.strokeWidth };
        if (isCubic(s)) { next.c1 = scalePt(s.c1!); next.c2 = scalePt(s.c2!); }
        return next;
      }
    }));
  };
  ...
  function strokeToPathSplitMid(s:Stroke):PathShape{
    if (isCubic(s)) {
      ...
      return { id: crypto.randomUUID(), type:'path', nodes,
        stroke:s.stroke, strokeWidth:s.strokeWidth, strokeLinecap:s.strokeLinecap, strokeLinejoin:s.strokeLinejoin };
    } else {
  ...
  function longestSegIndex(pth:PathShape){
    ...
    for(let i=0;i<pth.nodes.length-1 + (pth.closed?1:0); i++){
      const a = pth.nodes[i % pth.nodes.length], b = pth.nodes[(i+1) % pth.nodes.length];
      const L = dist(a.p,b.p);
      if (L>bestLen){ bestLen=L; best=i % pth.nodes.length; }
    }
    return best;
  }
  function splitPathAt(path:PathShape, segIdx:number, t:number):PathShape{
    const nodes = path.nodes.slice();
    const a = nodes[segIdx], b = nodes[(segIdx+1)%nodes.length];
  ...
  const addNodeMid = ()=>{
    ...
    if (!isPath(selectedShape)) {
      ...
      setShapes(prev => prev.map(x=>x.id===selectedShape.id? splitPathAt(selectedShape, idx, 0.5) : x));
    }
    setMode('points');
  };
  ...
  return (
    <div style={{ padding: 12, background: '#f5f5f5', borderRadius: 12 }}>
      ...
      <div style={{ display:'flex', gap:8, alignItems:'center', marginBottom:10, padding:8, background:'#fff', border:'1px solid #eee', borderRadius:8 }}>
        <strong style={{ color:'#111' }}>Canvas</strong>
        <select onChange={(e)=>onPickPreset(Number(e.target.value))} value={sizePresets.findIndex(p=>p.w===canvasW&&p.h===canvasH)}>
          {sizePresets.map((p, i)=>(
            <option key={p.label} value={i}>{p.label}</option>
          ))}
        </select>
        <span style={{ color:'#666' }}>{canvasW} × {canvasH}px</span>
      </div>

      <div style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems:'center', flexWrap:'wrap' }}>
        {/* 모드 토글 */}
        <span style={{ display:'inline-flex', border:'1px solid #ddd', borderRadius:8, overflow:'hidden' }}>
          <button
            onClick={()=>setMode('select')}
            style={{ padding:'6px 10px', background: mode==='select' ? '#111' : '#fff', color: mode==='select' ? '#fff' : '#111', border:'none' }}>
            Select
          </button>
          <button
            onClick={()=>setMode('points')}
            style={{ padding:'6px 10px', background: mode==='points' ? '#111' : '#fff', color: mode==='points' ? '#fff' : '#111', border:'none' }}>
            Edit points
          </button>
        </span>
        ...
        <button onClick={toggleCubic} disabled={!selectedShape || selIsPath}>Curve Toggle (C)</button>
        <button onClick={mergeSelected} disabled={!selectedShape || selIsPath}>Merge to Path (M)</button>
        <button onClick={addNodeMid} disabled={!selectedShape}>Add Node (A)</button>
        <button onClick={deselect} disabled={!selectedShape}>Deselect</button>
        <button onClick={removeSelected} disabled={!selectedShape}>Delete (⌫)</button>
        <button onClick={saveJSON}>Save JSON (console)</button>

        {/* 스타일 & 스케일 — 항상 보이고, 선택 없으면 비활성 */}
        <span style={{ marginLeft: 12, color:'#333' }}>— Style —</span>
        <StylePanel selectedShape={selectedShape} disabled={!selectedShape} onChange={updateStyle} />
        <span style={{ marginLeft: 12, color:'#333', opacity:!selectedShape?0.5:1 }}>— Scale —</span>
        <ScaleControls onScale={(f, withStroke)=>scaleSelected(f, withStroke)} disabled={!selectedShape} />
      </div>

      <svg
        width="100%" height={canvasH} viewBox={`0 0 ${canvasW} ${canvasH}`}
        style={{ background: '#fff', border: '1px solid #ddd', borderRadius: 12 }}
        {...bindCanvas()}
      >
        <defs>
          <clipPath id="canvasClip"><rect x="0" y="0" width={canvasW} height={canvasH} /></clipPath>
          <pattern id="grid" width={GRID} height={GRID} patternUnits="userSpaceOnUse">
            <path d={`M ${GRID} 0 L 0 0 0 ${GRID}`} fill="none" stroke="#eee" strokeWidth="1" vectorEffect="non-scaling-stroke"/>
          </pattern>
        </defs>

        {/* 배경(클릭 시 선택 해제) */}
        <rect x="0" y="0" width={canvasW} height={canvasH} fill="url(#grid)" onMouseDown={() => setSelected(null)} />
        {/* 세이프 박스(빨간 박스) */}
        <rect
          x={inset} y={inset} width={safeW} height={safeH}
        ...
        {mode==='points' && shapes.map(s => {
          if (selected !== s.id) return null;

          if (isPath(s)) {
            return (
              <g key={`handles-path-${s.id}`} onMouseDown={(e)=>e.stopPropagation()}>
                {s.nodes.map((n, i) => (
                  <g key={i}>
                    {n.h1 && <path d={`M ${n.p.x} ${n.p.y} L ${n.h1.x} ${n.h1.y}`} stroke="#f59e0b" fill="none" opacity={0.6} vectorEffect="non-scaling-stroke"/>}
                    {n.h2 && <path d={`M ${n.p.x} ${n.p.y} L ${n.h2.x} ${n.h2.y}`} stroke="#f59e0b" fill="none" opacity={0.6} vectorEffect="non-scaling-stroke"/>}
                    {n.h1 && <Handle p={n.h1} fill="#f59e0b" stroke="#b45309" onDrag={(dx,dy,m)=>movePathH(s.id,i,'h1',dx,dy,m)} onDragEnd={commitNoop} label={`h1`} />}
                    <Handle p={n.p} fill="#fff" stroke="#111" onDrag={(dx,dy,m)=>movePathP(s.id,i,dx,dy,m)} onDragEnd={commitNoop} label={`p${i}`} />
                    {n.h2 && <Handle p={n.h2} fill="#f59e0b" stroke="#b45309" onDrag={(dx,dy,m)=>movePathH(s.id,i,'h2',dx,dy,m)} onDragEnd={commitNoop} label={`h2`} />}
                  </g>
                ))}
              </g>
            );
          ...
          return (
            <g key={`handles-stroke-${s.id}`} onMouseDown={(e) => { e.stopPropagation(); setSelected(s.id); }}>
              {isCubic(st) && (
                <>
                  <path d={`M ${st.p0.x} ${st.p0.y} L ${st.c1!.x} ${st.c1!.y}`} stroke="#f59e0b" fill="none" opacity={0.6} vectorEffect="non-scaling-stroke" />
                  <path d={`M ${st.p1.x} ${st.p1.y} L ${st.c2!.x} ${st.c2!.y}`} stroke="#f59e0b" fill="none" opacity={0.6} vectorEffect="non-scaling-stroke"/>
                </>
              )}
              <Handle p={st.p0} fill="#fff" stroke="#111" onDrag={(dx,dy,m)=>movePoint(st.id,'p0',dx,dy,m)} onDragEnd={(_,__,m)=>commitPoint(st.id,'p0',m)} label="p0" />
              {isCubic(st) && (
                <Handle p={st.c1!} fill="#f59e0b" stroke="#b45309" onDrag={(dx,dy,m)=>movePoint(st.id,'c1',dx,dy,m)} onDragEnd={(_,__,m)=>commitPoint(st.id,'c1',m)} label="c1" />
              )}
              {isCubic(st) && (
                <Handle p={st.c2!} fill="#f59e0b" stroke="#b45309" onDrag={(dx,dy,m)=>movePoint(st.id,'c2',dx,dy,m)} onDragEnd={(_,__,m)=>commitPoint(st.id,'c2',m)} label="c2" />
              )}
              <Handle p={st.p1} fill="#fff" stroke="#111" onDrag={(dx,dy,m)=>movePoint(st.id,'p1',dx,dy,m)} onDragEnd={(_,__,m)=>commitPoint(st.id,'p1',m)} label="p1" />
            </g>
          );
        })}
      </svg>
...
// Panels & Utils
function StylePanel({
  selectedShape, disabled, onChange
}:{
  selectedShape: Shape | null;
  disabled: boolean;
  onChange: (patch: Partial<Pick<Shape,'stroke'|'strokeWidth'|'strokeLinecap'|'strokeLinejoin'>>) => void;
}){
  return (
    <>
      <label style={{ display:'inline-flex', alignItems:'center', gap:4, opacity:disabled?0.5:1 }}>
        Width
...
function ScaleControls({ onScale, disabled }:{ onScale:(factor:number, withStroke:boolean)=>void; disabled?:boolean }) {
  const [withStroke, setWithStroke] = useState(true);
  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:6, opacity:disabled?0.5:1 }}>
      <label style={{ display:'inline-flex', alignItems:'center', gap:4 }}>
        <input type="checkbox" checked={withStroke} onChange={e=>setWithStroke(e.target.checked)} disabled={disabled}/>
        scale stroke width
      </label>
      <button onClick={()=>onScale(0.9, withStroke)} disabled={disabled}>Scale -10%</button>
      <button onClick={()=>onScale(1.1, withStroke)} disabled={disabled}>Scale +10%</button>
    </span>
  );
}

// util: 복제/오프셋 (현재 캔버스 clamp 사용)
function withNewId<T extends Shape>(s: T): T { const copy = JSON.parse(JSON.stringify(s)) as T; (copy as any).id = crypto.randomUUID(); return copy; }
function offsetShape<T extends Shape>(s: T, dx: number, dy: number, clampByKey:(k:'p0'|'p1'|'c1'|'c2',pt:Pt)=>Pt): T {
  if (isPath(s)) {
    const nodes = s.nodes.map(n => ({
