...
type Stroke = { id:number,p0: Pt; p1: Pt; color?: string; strokeWidth?: number };
...
// stroke객체 업데이트하는 함수
const updatePoint = (stroke: Stroke, pointType: string, delta: [number, number]) => {
    if (pointType === 'p0') {
        return { ...stroke, p0: { x: stroke.p0.x + delta[0], y: stroke.p0.y + delta[1] } };
    }
    if (pointType === 'p1') {
        return { ...stroke, p1: { x: stroke.p1.x + delta[0], y: stroke.p1.y + delta[1] } };
    }
    return { ...stroke,
       ...
       <g key={i.id} >
        {
            ...
            />
            {
                 ...
                 <>
                    <path d={drawCircle(6,i.p0.x,i.p0.y)} fill='#fff' stroke={INITIAL_COLOR} strokeWidth={INITIAL_COLOR} data-stroke-id={i.id} data-point-type="p0" {...bind()} onClick={(e) => e.stopPropagation()}/>
                    <path d={drawCircle(6,i.p1.x,i.p1.y)} fill='#fff' stroke={INITIAL_COLOR} strokeWidth={INITIAL_COLOR} data-stroke-id={i.id} data-point-type="p1" {...bind()} onClick={(e) => e.stopPropagation()}/>
                 </>
  ...
  return (
    <div>
      ...
      <BtnWrapper>
        ...
        <button onClick={deleteStroke} disabled={selectedStrokeId===null}>stroke 삭제</button>
      </BtnWrapper>
      {/* svg 내에서 path 사용이 가능 */}
      <svg width="500px" height="500px" style={{border: '1px solid #111',backgroundColor:"#fff"}} onClick={()=>setSelectedStrokeId(null)}>
        {
            paintStrokes()
        }
      </svg>
